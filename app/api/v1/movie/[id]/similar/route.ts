...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    // cinezo's `/similar` returns unrelated popular titles; `/recommendations`
    // returns actually-similar movies, so we proxy that instead.
    const data = await tmdb<any>(`/movie/${id}/recommendations?page=${page}`);
