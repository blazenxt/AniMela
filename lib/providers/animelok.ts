...
/**
 * Return the deduped iframe-embed server options for an episode.
 *
 ...
 * button. We expose one embed server per host (HD-1 / HD-2).
 *
 * NOTE: we deliberately do NOT return direct m3u8 here — flixcloud streams are
 * IP-bound to the decryptor (our datacenter IP) and their CDN Cloudflare-blocks
 * datacenter IPs, so direct playback 403s. The iframe embed is the reliable
 * path (it plays on the user's IP).
 */
